...
begin
  Memo1.Lines.Insert(0, Format('[%s] %s', [FormatDateTime('hh:mm:ss.zzz', now), 'ERROR discovering devices']));
end;
...
begin
  ...
  Stop;
  Memo1.Lines.Insert(0, Format('[%s] %s', [FormatDateTime('hh:mm:ss.zzz', now), 'HRM disconnected']));
end;

procedure TStreamingDeviceFrame.GenericHRMDeviceConnected;
begin
  ActionStartStop.Enabled := true;
  Memo1.Lines.Insert(0, Format('[%s] %s', [FormatDateTime('hh:mm:ss.zzz', now), 'HRM found']));
end;
